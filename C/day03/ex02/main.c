...
int main(int ac, char **av)
{
	...
	node1 = findParent(root, "Dinosauria", "Homo sapiens");
	node2 = findParent(root, "Lynx", "Marsupialia");
	node3 = findParent(root, "Dinosauria", "I do not exist!");
	if (ac == 3)
		node1 = findParent(root, av[1], av[2]);
	if (node1)
		printf("%s\n", node1->name);
	if (node2)
		printf("%s\n", node2->name);
